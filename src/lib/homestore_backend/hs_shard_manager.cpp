...
void HSHomeObject::add_new_shard_to_map(std::unique_ptr< HS_Shard > shard) {
    ...
    auto [it, h] = chunk_to_shards_map_.try_emplace(shard->p_chunk_id(), {});
    it->second.emplace(shard_id);
...
const HS_Shard* HSHomeObject::_get_hs_shard(const shard_id_t shard_id) const {
    std::scoped_lock lock_guard(_shard_lock);
    auto shard_iter = _shard_map.find(shard_id);
    if (shard_iter == _shard_map.end()) { return nullptr; }
    return d_cast< HS_Shard* >((*shard_iter->second).get());
}
