...
protected:
    struct IPCData {
        void sync(uint64_t sync_point, uint32_t max_count) {
            static constexpr auto k_sync_timeout = std::chrono::minutes(30);
            ...
            if (homeobject_replica_count_ == max_count) {
                ...
                RELEASE_ASSERT(ok, "sync timed out after 30min at sync_point={}, a replica likely crashed", sync_point);
            }
        }
