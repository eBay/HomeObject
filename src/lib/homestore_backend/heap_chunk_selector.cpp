...
uint64_t HeapChunkSelector::get_used_blks() const {
    ...
    for (const auto& [_, pg_chunk_collection] : m_per_pg_chunks) {
        for (const auto& chunk : pg_chunk_collection->m_pg_chunks) {
            if (chunk->m_state == ChunkState::INUSE) { used_blks += chunk->get_used_blks(); }
        }
    }
    return used_blks;
}
