...
bool GCManager::pdev_gc_actor::replace_blob_index(
    ...
    for (const auto& [k, v] : valid_blob_indexes) {
        ...
        homestore::BtreeSinglePutRequest update_req{
            ...
            [&pg_id, &shard, &blob, task_id](homestore::BtreeKey const& key,
                                             homestore::BtreeValue const& value_in_btree,
        ...
        LOGDEBUG("gc task_id={},update index table for pg={}, ret={}, move_from_chunk={}, move_to_chunk={}, shard={}, "
                 "blob={}",
...
bool GCManager::pdev_gc_actor::copy_valid_data(chunk_id_t move_from_chunk, chunk_id_t move_to_chunk,
    ...
    for (const auto& shard_id : shards) {
        ...
        if (valid_blob_indexes.empty()) {
            ...
            LOGDEBUG("gc task_id={}, {} valid blobs found in move_from_chunk, chunk_id={}, shard_id={}",
                     valid_blob_indexes.size(), task_id, move_from_chunk, shard_id);
        }
    ...
    if (data_service.commit_blk(commit_blk_id) != homestore::BlkAllocStatus::SUCCESS) {
        LOGERROR("gc task_id={}, fail to commit_blk for move_to_chunk={}, move_from_chunk={}", task_id, move_to_chunk,
                 move_from_chunk);
        return false;
    }
