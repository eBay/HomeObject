...
public:
    ...
    void cancel() {
        ...
        if (!cancelled.compare_exchange_strong(is_cancelled, true)) {
            SCRUBLOGI(pg_id, task_id, "scrub task is already cancelled, no need to cancel again");
            return;
        }
...
private:
    ...
    struct PeerBatchScrubCtx {
        scrub_req current_req;
    ...
    // we have an assumption that a scrub task is always handled by a single thread, so the key will never change after
    // it is initialized. so we can safely read m_active_batch or iterate through m_active_batch without lock.
    std::map< peer_id_t, std::shared_ptr< PeerBatchScrubCtx > > m_active_batch;
...
void ScrubManager::check_scrub_timeouts() {
    ...
    for (auto const& [pg_id, scrub_ctx] : m_pg_scrub_ctx_map) {
        ...
        for (auto const& [peer_id, peer_batch_scrub_ctx] : scrub_ctx->m_active_batch) {
            ...
            if (std::chrono::steady_clock::now() - peer_batch_scrub_ctx->last_sent_at < timeout) {
                SCRUBLOGD(pg_id, task_id, "batch scrub for peer {} has not timed out yet, skipping", peer_id);
                continue;
            }

            peer_batch_scrub_ctx->retry_count++;
            SCRUBLOGW(pg_id, task_id, "timeout waiting for scrub result from peer {}, req_id={}, retry={}/{}", peer_id,
                      peer_batch_scrub_ctx->current_req.req_id, peer_batch_scrub_ctx->retry_count, max_retries);

            if (peer_batch_scrub_ctx->retry_count > max_retries) {
                lock.unlock();
                scrub_ctx->cancel();
                break;
            }

            peer_batch_scrub_ctx->last_sent_at = std::chrono::steady_clock::now();
            const auto next_req = peer_batch_scrub_ctx->current_req;
            lock.unlock();
...
void ScrubManager::add_scrub_req(std::shared_ptr< scrub_req > req) {
    ...
    LOGWARNMOD(scrubmgr, "scrub manager is not started yet, drop this req, req_id={}!", req->req_id);
}
...
std::shared_ptr< ScrubManager::scrub_result > ScrubManager::local_scrub_meta(std::shared_ptr< scrub_req > req) {
    ...
    // for the first batch of meta scrub, we add a pg meta scrub_result_entry for it if start_shard_id is 0, so we need
    // to decrease the batch_capacity by 1 to make sure the total scrubbed shard count in this batch does not exceed
    // max_scrub_batch_size and not introduce an error in when handling scrub resp.
    uint32_t batch_capacity = max_scrub_batch_size - meta_scrub_result->entries.size();
...
ScrubManager::PGScrubContext::PGScrubContext(uint64_t task_id, const HSHomeObject::HS_PG* hs_pg) :
    ...
    // Build m_active_batch once; PG membership is stable for the lifetime of this scrub task.
    for (const auto& member : hs_pg->pg_info_.members) {
        m_active_batch.emplace(member.id, std::make_shared< PeerBatchScrubCtx >());
    }
...
void ScrubManager::PGScrubContext::handle_scrub_req_resp(std::shared_ptr< ScrubManager::scrub_result > result) {
    ...
    // m_active_batch keys are fixed after construction, so find without lock is safe.
    auto it = m_active_batch.find(result->issuer_peer_id);
    if (it == m_active_batch.end()) {
        SCRUBLOGD(pg_id, task_id, "no active batch for peer {}, dropping result req_id={}", result->issuer_peer_id,
                  result->req_id);
        return;
    }
    auto& peer_batch_scrub_ctx = it->second;
    ...
    // Drop stale responses from a previous sub-request send.
    if (result->req_id != peer_batch_scrub_ctx->current_req.req_id) {
        SCRUBLOGD(pg_id, task_id, "stale req_id from peer {}, expected={}, actual={}, dropping", result->issuer_peer_id,
                  peer_batch_scrub_ctx->current_req.req_id, result->req_id);
        return;
    }

    const auto received_batch_size = static_cast< uint32_t >(result->entries.size());
    const auto last_shard = received_batch_size > 0 ? result->entries.rbegin()->first.shard : uint64_t{0};
    const auto last_blob = received_batch_size > 0 ? result->entries.rbegin()->first.blob : uint64_t{0};

    peer_batch_scrub_ctx->batch_scrub_result->add_scrub_result(*result);
    // result->entries is now empty after merge.
    const auto& cur = peer_batch_scrub_ctx->current_req;

    // we only set a limitation for deep blob scrub since it is more expensive and we want to control the scrub load on
    // the system.
    const uint32_t batch_capacity = (cur.scrub_type == SCRUB_TYPE::DEEP_BLOB)
        ? static_cast< uint32_t >(deep_blob_scrub_batch_size)
        : static_cast< uint32_t >(max_scrub_batch_size);

    if (received_batch_size == batch_capacity) {
        bool peer_done = false;
        // Full batch: advance the frontier and issue the next sub-request.
        if (cur.scrub_type == SCRUB_TYPE::META) {
            peer_batch_scrub_ctx->current_req.start_shard_id = last_shard + 1;
            peer_done = (peer_batch_scrub_ctx->current_req.start_shard_id > cur.end_shard_id);
        } else {
            if (last_blob == cur.end_blob_id) {
                peer_batch_scrub_ctx->current_req.start_shard_id = last_shard + 1;
                peer_batch_scrub_ctx->current_req.start_blob_id = 0;
            } else {
                peer_batch_scrub_ctx->current_req.start_shard_id = last_shard;
                peer_batch_scrub_ctx->current_req.start_blob_id = last_blob + 1;
            }
            peer_done = (peer_batch_scrub_ctx->current_req.start_shard_id > cur.end_shard_id);
        }

        if (!peer_done) {
            peer_batch_scrub_ctx->current_req.req_id = random_req_id();
            peer_batch_scrub_ctx->retry_count = 0;
            peer_batch_scrub_ctx->last_sent_at = std::chrono::steady_clock::now();
            const auto next_req = peer_batch_scrub_ctx->current_req;
            lock.unlock();
            send_req_to_peer(next_req, result->issuer_peer_id);
            return;
        }
    }

    RELEASE_ASSERT(received_batch_size <= batch_capacity,
                   "peer {} sent batch result with size {} exceeding capacity {}, req_id={}", result->issuer_peer_id,
                   received_batch_size, batch_capacity, result->req_id);

    const auto& peer_batch_scrub_result = peer_batch_scrub_ctx->batch_scrub_result;
...
std::string ScrubManager::scrub_req::to_string() const {
    std::stringstream ss;
    ss << "pg_id=" << pg_id << ", req_id=" << req_id << ", scrub_lsn=" << scrub_lsn
       << ", start_shard_id=" << start_shard_id << ", start_blob_id=" << start_blob_id
       << ", end_shard_id=" << end_shard_id << ", end_blob_id=" << end_blob_id << ", issuer_peer_id=" << issuer_peer_id
       << ", scrub_type=" << static_cast< uint32_t >(scrub_type);
    return ss.str();
}
