...
// exit_pg is called when a pg/repl_dev is leaked, either during member leaving or a member failed to join the cluster. This function will try to destroy both pg and repl_dev.
// Since pg and repl_dev may be inconsistent, so ignore not found error to make it idempotent and retriable.
// Although pg_destroy will be called in the on_destroy cb when leaving the group, we still need to call pg_destroy here to make sure pg is cleaned in case of repl_dev destroy failed(http://github.com/eBay/HomeStore/issues/823).
