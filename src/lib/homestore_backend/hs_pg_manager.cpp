...
void HSHomeObject::update_pg_meta_after_gc(const pg_id_t pg_id, const homestore::chunk_num_t move_from_chunk,
    ...
    if (pg_chunks[v_chunk_id] == move_to_chunk) {
        ...
        // metablk is updated.
        LOGI("the pchunk_id for vchunk_id={} for pg_id={} is already {}, skip update pg metablk", v_chunk_id, pg_id,
             move_to_chunk);
    }
    ...
    // implemented later
    hs_pg->durable_entities_update([this, move_from_v_chunk, &move_to_chunk, &pg_id](auto& de) {
