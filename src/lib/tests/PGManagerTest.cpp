...
TEST_F(TestFixture, CreatePgEmpty) {
    EXPECT_EQ(sisl::async::sync_get(homeobj_->pg_manager()->create_pg(PGInfo(0u))).error(), PGError::INVALID_ARG);
}

TEST_F(TestFixture, CreatePgNoLeader) {
    auto info = PGInfo(0u);
    info.members.insert(PGMember{boost::uuids::random_generator()()});
    EXPECT_EQ(sisl::async::sync_get(homeobj_->pg_manager()->create_pg(std::move(info))).error(), PGError::INVALID_ARG);
}
...
TEST_F(TestFixture, Migrate) {
    ...
    EXPECT_EQ(sisl::async::sync_get(
                  homeobj_->pg_manager()->replace_member(UINT16_MAX, task_id, boost::uuids::random_generator()(),
                                                         PGMember{boost::uuids::random_generator()()}, 0))
                  .error(),
              PGError::UNKNOWN_PG);
    EXPECT_EQ(sisl::async::sync_get(homeobj_->pg_manager()->replace_member(_pg_id, task_id, _peer1, PGMember{_peer1}))
                  .error(),
              PGError::INVALID_ARG);
